...
CREATE OR REPLACE TABLE MEMBERS
    USING TEMPLATE (
        ...
        FROM TABLE(INFER_SCHEMA(LOCATION => '@DATA/members.csv', FILE_FORMAT => 'CSV_FORMAT'))
    );

CREATE OR REPLACE TABLE CLAIMS
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT('COLUMN_NAME', UPPER("COLUMN_NAME"), 'TYPE', "TYPE", 'NULLABLE', "NULLABLE"))
        FROM TABLE(INFER_SCHEMA(LOCATION => '@DATA/claims.csv', FILE_FORMAT => 'CSV_FORMAT'))
    );

CREATE OR REPLACE TABLE PROVIDERS
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT('COLUMN_NAME', UPPER("COLUMN_NAME"), 'TYPE', "TYPE", 'NULLABLE', "NULLABLE"))
        FROM TABLE(INFER_SCHEMA(LOCATION => '@DATA/providers.csv', FILE_FORMAT => 'CSV_FORMAT'))
    );

CREATE OR REPLACE TABLE DENTAL_PROCEDURES
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT('COLUMN_NAME', UPPER("COLUMN_NAME"), 'TYPE', "TYPE", 'NULLABLE', "NULLABLE"))
        FROM TABLE(INFER_SCHEMA(LOCATION => '@DATA/dental_procedures.csv', FILE_FORMAT => 'CSV_FORMAT'))
    );

CREATE OR REPLACE TABLE CLAIM_NOTES
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT('COLUMN_NAME', UPPER("COLUMN_NAME"), 'TYPE', "TYPE", 'NULLABLE', "NULLABLE"))
        FROM TABLE(INFER_SCHEMA(LOCATION => '@DATA/claim_notes.csv', FILE_FORMAT => 'CSV_FORMAT'))
    );

CREATE OR REPLACE TABLE MEMBER_COMMUNICATIONS
    USING TEMPLATE (
        SELECT ARRAY_AGG(OBJECT_CONSTRUCT('COLUMN_NAME', UPPER("COLUMN_NAME"), 'TYPE', "TYPE", 'NULLABLE', "NULLABLE"))
        FROM TABLE(INFER_SCHEMA(LOCATION => '@DATA/member_communications.csv', FILE_FORMAT => 'CSV_FORMAT'))
    );

-- 3. Load data into tables
COPY INTO MEMBERS FROM @DATA/members.csv FILE_FORMAT = 'CSV_FORMAT' MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
COPY INTO CLAIMS FROM @DATA/claims.csv FILE_FORMAT = 'CSV_FORMAT' MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
COPY INTO PROVIDERS FROM @DATA/providers.csv FILE_FORMAT = 'CSV_FORMAT' MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
COPY INTO DENTAL_PROCEDURES FROM @DATA/dental_procedures.csv FILE_FORMAT = 'CSV_FORMAT' MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
COPY INTO CLAIM_NOTES FROM @DATA/claim_notes.csv FILE_FORMAT = 'CSV_FORMAT' MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
COPY INTO MEMBER_COMMUNICATIONS FROM @DATA/member_communications.csv FILE_FORMAT = 'CSV_FORMAT' MATCH_BY_COLUMN_NAME = CASE_INSENSITIVE;
```
